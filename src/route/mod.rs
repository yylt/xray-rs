...
use crate::common::{domain_trie, ip_trie};
pub use resolver::{DnsResolver, DnsSettings};
...
impl RoutingSettings {
    pub fn build_router(&self, dns: Arc<DnsResolver>) -> io::Result<Router> {
        let mut domain_builder = domain_trie::DomainSuffixTrieBuilder::new(); //DomainMarisaBuilder::new();
        let mut ip_builder = ip_trie::IpTrieBuilder::new(); //IpTrieBuilder::new();
        ...
        let mut router = Router::new_with_tries(
            self.domain_strategy.clone(),
            domain_builder.build().expect("FST build failed"),
            ip_builder.build(),
        )
        .with_dns(dns);
...
fn add_domain_rule(
    builder: &mut domain_trie::DomainSuffixTrieBuilder,
    domain: &str,
    outbound_tag: &str,
) -> io::Result<()> {
...
fn add_domain_rule_entry(builder: &mut domain_trie::DomainSuffixTrieBuilder, domain: &str, outbound_tag: &str) {
    let domain = domain.trim();
    builder.insert(domain, outbound_tag);
}

fn add_ip_rule(builder: &mut ip_trie::IpTrieBuilder, ip_rule: &str, outbound_tag: &str) -> io::Result<()> {
...
fn add_ip_rules_from_file(builder: &mut ip_trie::IpTrieBuilder, file_path: &str, outbound_tag: &str) -> io::Result<()> {
