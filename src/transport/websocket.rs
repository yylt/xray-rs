...
mod tests {
    ...
    fn test_websocket_new() {
        use crate::route::{DnsResolver, DnsSettings};
        use std::sync::Arc;
        ...
        let dns = Arc::new(DnsResolver::new(dns_settings).unwrap());
        ...
        let ws = WebSocket::new(&stream_settings, dns);
        assert!(ws.is_ok());
    }

    #[test]
    fn test_websocket_new_without_settings() {
        use crate::route::{DnsResolver, DnsSettings};
        use std::sync::Arc;
        ...
        let ws = WebSocket::new(&stream_settings, dns);
        assert!(ws.is_ok()); // Should use defaults
    }

    #[test]
    fn test_websocket_settings_custom_path() {
        let mut settings = WsSettings::default();
        settings.path = "/custom".to_string();
