...
struct BalancerPool {
    targets: HashMap<GrpcTargetKey, Arc<TargetState>, RandomState>,
...
impl GrpcBalancer {
    ...
    pub async fn sync_targets(&self, new_targets: Vec<GrpcTargetKey>) {
        ...
        for (target, state) in pool.targets.iter() {
            if !new_set.contains(target) {
                state.set_draining(true);
            }
        }

        pool.targets.retain(|_, state| !(state.is_draining() && state.inflight() == 0));
    ...
    fn select_round_robin(
        ...
        for offset in 0..len {
            ...
            let state = pool.targets.get(key)?;
            if excluded.contains(key) || state.is_draining() {
                continue;
            }
    ...
    {
        let total_targets = self.pool.read().await.targets.len();
        if total_targets == 0 {
